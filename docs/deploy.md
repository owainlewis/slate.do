...
| --- | --- | --- |
| `slate-deploy` | Cloud Build and deploy | Cloud Build execution, Cloud Run and Scheduler deployment, Cloud Logging, the `slate` Artifact Registry repository, object and metadata access on the dedicated build bucket, and `actAs` only for the three runtime identities |
